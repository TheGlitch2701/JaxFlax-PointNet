...
def create_momentum_fn():
    """
    Creates and returns an exponential momentum decay schedule function.

    Returns:
        optax.Schedule: A function that computes exponentially decayed momentum 
                        values over training steps, used typically for batch 
                        normalization or optimizer momentum.
    """
    exp_fn = optax.exponential_decay(
        init_value=BN_INIT_DECAY,           # e.g., 0.5
        decay_rate=BN_DECAY_DECAY_RATE,     # e.g., 0.5
        staircase=True,
        transition_steps=BN_DECAY_DECAY_STEP)  # e.g., 200000

    return exp_fn

def create_learning_rate_fn():
    """
    Creates and returns an exponential learning rate decay schedule.

    Returns:
        optax.Schedule: A function that computes the learning rate at each step based on 
                        an exponential decay formula, used to adjust the optimizer's 
...
class TrainState(train_state.TrainState):
    """
    Custom training state that extends Flax's TrainState to include batch statistics and PRNG key.

    Args:
        train_state (train_state.TrainState): Base training state from Flax.

    Attributes:
        batch_stats (Any): Contains running statistics (e.g., for batch normalization).
        key (jax.Array): PRNG key used for stochastic operations during training.
    """
    batch_stats: Any
    key: jax.Array
...
def create_train_state(module, rng, dropout_key, optimizer):
    """
    Initializes and returns a custom training state for a given model and optimizer.

    This function initializes the model parameters, sets up the optimizer based on the 
    chosen type (momentum or Adam), and returns a TrainState object containing 
    the model parameters, batch statistics, and the PRNG key for random operations.

    Args:
        module (flax.linen.Module): The model to be trained.
        rng (jax.Array): The random number generator state for initializing the model.
        dropout_key (jax.Array): The PRNG key used for dropout during training.
        optimizer (str): The type of optimizer to use. Can be 'momentum' or 'adam'.

    Returns:
        TrainState: A custom training state that includes the model parameters, batch statistics,
...
def train_step(state: train_state.TrainState, X_batch, Y_batch, bn):
    """
    Performs a single training step, computing the loss, gradients, and updating the model parameters.

    This function computes the loss and its gradients with respect to the model parameters,
    applies the gradients to update the parameters, and returns the updated training state.

    Args:
        state (train_state.TrainState): The current state of the model, including parameters, 
                                        batch statistics, and PRNG key.
        X_batch (jax.Array): The input data for the current batch.
        Y_batch (jax.Array): The target labels for the current batch.
        bn (float): Batch normalization decay value to adjust momentum during training.

    Returns:
        train_state.TrainState: The updated training state with updated model parameters and batch statistics.
        float: The computed loss for the current batch.
        int: The number of correct predictions in the current batch.
    """
    ...
    def loss_fn(params, x, c, bn, reg_weights=1e-3):
        """
        Computes the loss and accuracy for a given set of parameters and inputs.

        Args:
            params (dict): Model parameters.
            x (jax.Array): Input data for the current batch.
            c (jax.Array): Target labels for the current batch.
            bn (float): Batch normalization decay value.
            reg_weights (float, optional): Weight for regularization loss. Defaults to 1e-3.

        Returns:
            tuple: A tuple containing the total loss (including regularization) and accuracy.
        """
        ...
        preds = jax.nn.softmax(final_layer, axis=1)  # (batch_size, num_classes)
        preds = jnp.argmax(preds, axis=1)  # Predicted class indices

        correct = jnp.sum(preds == c)  # Count the number of correct predictions
        loss = optax.losses.softmax_cross_entropy(logits=final_layer, labels=c_one_hot).mean(axis=0)  # Scalar loss
...
def eval_step(state: train_state.TrainState, X_batch, Y_batch, bn, reg_weights=1e-3):
    """
    Performs a single evaluation step, computing the loss and accuracy for a given batch.

    This function evaluates the model on the provided batch, computing the softmax cross-entropy
    loss and the number of correct predictions. It also calculates a regularization term based on 
    ...
    Args:
        ...
        X_batch (jax.Array): The input data for the current batch.
        Y_batch (jax.Array): The target labels for the current batch.
        bn (float): Batch normalization decay value to adjust momentum during evaluation.
        reg_weights (float, optional): Weight for regularization loss. Defaults to 1e-3.

    Returns:
        float: The total loss (including regularization) for the current batch.
        int: The number of correct predictions in the current batch.
        jax.Array: The predicted class indices for the current batch.
    """
    ...
    preds = jax.nn.softmax(logits, axis=1)  # (batch_size, num_classes)
    pred_choice = jnp.argmax(preds, axis=1)  # Predicted class indices

    correct = jnp.sum(pred_choice == Y_batch)  # Count the number of correct predictions
    loss = optax.losses.softmax_cross_entropy(logits=logits, labels=c_one_hot).mean(axis=0)  # Scalar loss
...
def TrainModelInBatches(X, Y, state, learning_rate_fn, momentum_fn):
    """
    Trains the model in batches and updates the model parameters using the specified learning rate
    and momentum schedules.

    This function iterates through the training data in batches, performs training steps, and 
    computes the loss and accuracy for each batch. The learning rate and momentum are updated 
    according to the provided schedules. The function logs the training loss and accuracy at the 
    end of the training process.

    Args:
        X (jax.Array): The input data for training, with shape (num_samples, num_features).
        Y (jax.Array): The target labels for training, with shape (num_samples,).
        state (train_state.TrainState): The current state of the model, including parameters,
                                        batch statistics, and PRNG key.
        learning_rate_fn (optax.Schedule): A function that computes the learning rate schedule.
        momentum_fn (optax.Schedule): A function that computes the momentum schedule.

    Returns:
        train_state.TrainState: The updated training state after training on all batches.
    ...
    for batch in tqdm(batches):  # Iterate over batches
        if batch != batches[-1]:
        ...
        # Updating learning rate and batch normalization history
        lr_story.append(lr)
        bn_story.append(bn)
...
def EvalModelInBatches(X, Y, state, total_correct, total_seen, total_correct_class, total_seen_class):
    """
    Evaluates the model on a given dataset in batches, computing the loss and accuracy,
    as well as the class-wise accuracy.

    This function iterates over the data in batches, evaluates the model for each batch, 
    and accumulates the total loss, correct predictions, and class-wise accuracies.

    Args:
        X (jax.Array): The input data for evaluation, with shape (num_samples, num_features).
        Y (jax.Array): The target labels for evaluation, with shape (num_samples,).
        state (train_state.TrainState): The current state of the model, including parameters,
                                        batch statistics, and PRNG key.
        total_correct (int): The running total of correct predictions across all batches.
        total_seen (int): The running total of samples processed across all batches.
        total_correct_class (list): A list that accumulates the number of correct predictions 
                                    for each class.
        total_seen_class (list): A list that accumulates the number of samples for each class.

    Returns:
        float: The total loss across all batches.
        int: The total number of correct predictions across all batches.
        int: The total number of samples processed across all batches.
        list: The total number of correct predictions for each class.
        list: The total number of samples seen for each class.
    """
...
def figures(train_loss, eval_loss, train_accuracy, eval_accuracy, lr_story, bn_story):
    """
    Plots and saves the training and validation loss and accuracy curves, along with learning 
    rate and batch normalization histories.

    This function creates a 2x2 subplot grid displaying the following:
    - Training and validation loss curves
    - Training and validation accuracy curves
    The learning rate and batch normalization histories are also passed but are not plotted in this function.

    Args:
        train_loss (list of float): A list containing the training loss values for each epoch.
        eval_loss (list of float): A list containing the validation loss values for each epoch.
        train_accuracy (list of float): A list containing the training accuracy values for each epoch.
        eval_accuracy (list of float): A list containing the validation accuracy values for each epoch.
        lr_story (list of float): A list tracking the learning rate at each training step (not used in plotting).
        bn_story (list of float): A list tracking the batch normalization values at each training step (not used in plotting).

    Returns:
        None: The function saves the plots as an image file and does not return any value.
    """
